...
path = r"C:\Users\mayan\Desktop\timetable project\FinalInput.xlsx"
parent_dir = r"C:\Users\mayan\Desktop\timetable project"
...
coursesCodes, facultyNames, courseyear, lecture, tutorial, practical = list(data.values())
...
Timings = ["9:00AM", "10:00AM", "11:00AM", "12:00PM",
           "1:00PM", "2:00PM", "3:00PM", "4:00PM"]
lunch = "1:00PM"
i = Timings.index(lunch)

# Class timings before and after lunch break
beforelunch = Timings[:i]
afterlunch = Timings[i+1:]
...
class course:
    # This is constructor for course class
    def __init__(self, code, faculty=None, year=None, credits=(0, 0, 0)):
        self.courseCode = code
        self.faculty = faculty
        # Note that year is not an integer, its and object
        self.year = year
        self.L, self.T, self.P = credits


    # For later use (if any facculty wants to teach or wants to remain free on a particular slot)
...
class facultywiseSchedule:
    ...
    def addPClass(self, CC, yearobj, day):
        ...
        if not self.check(CC, yearobj, day):
            if self.isavailable(day, time) and self.isavailable(day, ntime) and yearobj.slotavailable(day, time) and yearobj.slotavailable(day, ntime):
                ...
                print(
                    f"{CC} course Practical by faculty {self.name} - assinged succesfully")
                logs_generated.write(
                    f"{CC} course Practical by faculty {self.name} - assinged succesfully\n")
                return i
            else:
                i = WorkingDays.index(day)
                return self.addPClass(CC, yearobj, WorkingDays[(i+1) % len(WorkingDays)])

        else:
            return self.addPClass(CC, yearobj, WorkingDays[(i+1) % len(WorkingDays)])

    def addClass(self, CC, yearobj, day, ltype):
...
courseobj = {}
for i in range(n):
    newcourse = course(coursesCodes[i], facultyNames[i], bscmath[courseyear[i]], credits=(
        lecture[i], tutorial[i], practical[i]))
        ...
        for time in Timings:
            if time != Timings[-1]:
                ...
                if F.schedule[day][time] != "LUNCH" and F.schedule[day][time] != 0:
                    ...
                    sheet.cell(row, 5).value = Ltype
                    TABLE_DATA.append(
                        [CC, courseyear[coursesCodes.index(CC)], day, f"{time} - {ntime}", sheet.cell(row, 5).value])
                    row += 1
